...
set fname = ch4.${irun}.tdz.ccsd.f12.gii
...
if (-e $fname.dat) then
   /bin/cp  $fname.dat  $pwd/dat-ccsd-f12-tz-II
endif

if (-e $fname.xyz.dat) then
   /bin/cp  $fname.xyz.dat  $pwd/dat-ccsd-f12-tz-xyz-II
endif

if (-e $pwd/$fname.out) then
    gzip $pwd/$fname.out
   /bin/mv  $pwd/$fname.out.gz  $pwd/out-ccsd-f12-tz-II
endif
